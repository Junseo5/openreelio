 ...
 */
async importStockMediaAsset(sourceUrl: string, name: string, assetType: string, provider: string, license: LicenseInfo, licenseAck: boolean, durationSec: number | null, tags: string[] | null, providerUrl: string | null) : Promise<Result<StockMediaImportResult, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("import_stock_media_asset", { sourceUrl, name, assetType, provider, license, licenseAck, durationSec, tags, providerUrl }) };
} catch (e) {
    return { status: "error", error: e  as any };
}
},
 ...
 */
export type StockMediaImportResult = {
/**
