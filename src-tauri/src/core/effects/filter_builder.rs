...
impl IntoFFmpegFilter for Effect {
    ...
    fn filter_name(&self) -> &'static str {
        match &self.effect_type {
            ...
            // Stabilization
            EffectType::Stabilize => "vidstabtransform",

            // AI smart reframe uses FFmpeg crop filter with dynamic positioning
...
impl Effect {
    ...
    pub(crate) fn build_filter_params(&self) -> String {
        match &self.effect_type {
            ...
            // Stabilization
            EffectType::Stabilize => self.build_stabilize_filter(),

            // AI smart reframe
            EffectType::AutoReframe => self.build_auto_reframe_filter(),
    ...
    /// Builds FFmpeg `vidstabtransform` filter for video stabilization.
    ///
    /// Requires a pre-computed transforms file (.trf) generated by the
    /// `vidstabdetect` analysis pass. The `analysis_path` parameter stores
    /// the path to this transforms file.
    ///
    /// # Parameters
    ///
    /// - `smoothing`: Motion smoothing strength (1-100, default: 10)
    /// - `crop_mode`: How to handle border artifacts — "none", "crop", "dynamic"
    ...
    // -------------------------------------------------------------------------
    // Auto Reframe (Smart Reframe) Builder
    // -------------------------------------------------------------------------

    /// Builds an FFmpeg `crop` filter that dynamically repositions the crop
    /// window based on pre-computed keyframes stored in `analysis_data`.
    ///
    /// The analysis pass (run via the `smart_reframe` IPC command) detects
    /// the region of interest per scene/segment and stores the result as a
    /// JSON object in the `analysis_data` parameter.
    ///
    ...
    /// # Parameters
    ///
    /// - `target_aspect`: Target aspect ratio string (e.g., "9:16", "1:1")
    /// - `smoothing`: Crop motion smoothing strength (1-100, default: 30)
    /// - `zoom`: Additional zoom percentage (0-50, default: 0)
    ...
    fn build_auto_reframe_filter(&self) -> String {
        ...
        // Parse analysis data
        let parsed: serde_json::Value = match serde_json::from_str(&analysis_data) {
            Ok(v) => v,
            Err(_) => return "null".to_string(),
        };

        let crop_w = parsed["crop_w"].as_i64().unwrap_or(0);
        let crop_h = parsed["crop_h"].as_i64().unwrap_or(0);

        if crop_w <= 0 || crop_h <= 0 {
            return "null".to_string();
        }

        // Apply zoom: reduce crop dimensions to zoom in further.
        // Re-align to even dimensions after zoom to avoid codec errors (H.264/H.265
        // require even width and height).
        let zoom = self.get_float("zoom").unwrap_or(0.0).clamp(0.0, 50.0);
        let zoom_factor = 1.0 - (zoom / 100.0);
    ...
    /// Builds a piecewise linear interpolation expression using FFmpeg's `if`
    /// and `between` functions.  Given keyframes sorted by time, produces:
    ///
    /// ```text
    /// if(lt(t,t1),v0, if(lt(t,t2),v1+(v2-v1)*(t-t1)/(t2-t1), ...))
    /// ```
    ///
    /// The first segment holds the initial value constant, and the last
    /// keyframe value is held for all time beyond the final keyframe.
    fn build_lerp_expression(keyframes: &[serde_json::Value], field: &str) -> String {
        // Sort keyframes by time to guarantee monotonic order.  The analysis
        // pass should already produce sorted keyframes, but callers may
        // supply manually-edited JSON where the order is not guaranteed.
        let mut sorted: Vec<&serde_json::Value> = keyframes.iter().collect();
        sorted.sort_by(|a, b| {
            let ta = a["t"].as_f64().unwrap_or(0.0);
            let tb = b["t"].as_f64().unwrap_or(0.0);
            ta.partial_cmp(&tb).unwrap_or(std::cmp::Ordering::Equal)
        });
        let keyframes = sorted;
        ...
        if len == 1 {
            return keyframes[0][field].as_i64().unwrap_or(0).to_string();
        }
        ...
        for i in 0..len - 1 {
            let t0 = keyframes[i]["t"].as_f64().unwrap_or(0.0);
            let t1 = keyframes[i + 1]["t"].as_f64().unwrap_or(0.0);
            let v0 = keyframes[i][field].as_i64().unwrap_or(0);
            let v1 = keyframes[i + 1][field].as_i64().unwrap_or(0);

            if i == 0 {
                // Hold first value before first keyframe
                expr.push_str(&format!("if(lt(t\\,{:.2})\\,{}", t0, v0));
                depth += 1;
            }
            ...
            let dt = t1 - t0;
            if dt.abs() < 0.001 {
                // Zero-length segment: snap to v1
                expr.push_str(&format!("\\,if(lt(t\\,{:.2})\\,{}", t1, v1));
            } else {
        ...
        // Hold last value beyond final keyframe
        let last_val = keyframes[len - 1][field].as_i64().unwrap_or(0);
        expr.push_str(&format!("\\,{}", last_val));
...
mod tests {
    ...
    // =========================================================================
    // Stabilize filter tests
    // =========================================================================

    #[test]
    fn test_stabilize_filter_without_analysis_path_returns_null() {
        let effect = Effect::new(EffectType::Stabilize);
    ...
    fn test_stabilize_filter_with_analysis_path() {
        ...
        effect.set_param(
            "analysis_path",
            ParamValue::String("/tmp/test.trf".to_string()),
        );
        effect.set_param("smoothing", ParamValue::Float(15.0));
        effect.set_param("crop_mode", ParamValue::String("crop".to_string()));
        effect.set_param("zoom", ParamValue::Float(5.0));

        let filter = effect.to_filter_body();
        assert!(
            filter.contains("vidstabtransform"),
            "Expected vidstabtransform, got: {}",
            filter
        );
        assert!(
            filter.contains("smoothing=15"),
            "Expected smoothing=15, got: {}",
            filter
        );
        assert!(
        ...
        assert!(
            filter.contains("zoom=5.0"),
            "Expected zoom=5.0, got: {}",
            filter
        );
        assert!(
            filter.contains("/tmp/test.trf"),
            "Expected transforms path, got: {}",
            filter
        );
    }

    #[test]
    fn test_stabilize_filter_crop_mode_none() {
        let mut effect = Effect::new(EffectType::Stabilize);
        effect.set_param(
            "analysis_path",
            ParamValue::String("/tmp/test.trf".to_string()),
        );
        effect.set_param("crop_mode", ParamValue::String("none".to_string()));
    ...
    #[test]
    fn test_stabilize_filter_crop_mode_dynamic() {
        let mut effect = Effect::new(EffectType::Stabilize);
        effect.set_param(
            "analysis_path",
            ParamValue::String("/tmp/test.trf".to_string()),
        );
        effect.set_param("crop_mode", ParamValue::String("dynamic".to_string()));
    ...
    #[test]
    fn test_stabilize_filter_smoothing_clamp() {
        let mut effect = Effect::new(EffectType::Stabilize);
        effect.set_param(
            "analysis_path",
            ParamValue::String("/tmp/test.trf".to_string()),
        );
        // Smoothing beyond max should clamp
        effect.set_param("smoothing", ParamValue::Float(200.0));

        let filter = effect.to_filter_body();
        assert!(
            filter.contains("smoothing=100"),
            "Expected smoothing clamped to 100, got: {}",
            filter
        );
    }

    #[test]
    fn test_stabilize_effect_category() {
        let effect = Effect::new(EffectType::Stabilize);
        assert_eq!(effect.effect_type.category(), EffectCategory::Transform);
    }

    #[test]
    fn test_stabilize_default_params() {
        let effect = Effect::new(EffectType::Stabilize);
        assert_eq!(
            effect.get_float("smoothing"),
            Some(10.0),
            "Default smoothing should be 10.0"
        );
        assert_eq!(
            effect.get_string("crop_mode"),
            Some("crop".to_string()),
            "Default crop_mode should be 'crop'"
        );
        assert_eq!(
            effect.get_float("zoom"),
            Some(0.0),
            "Default zoom should be 0.0"
        );
    }

    #[test]
    fn test_stabilize_is_ffmpeg_compatible() {
        let effect = Effect::new(EffectType::Stabilize);
        assert!(
            effect.is_ffmpeg_compatible(),
            "Stabilize should be FFmpeg compatible"
        );
    }

    // =========================================================================
    // AutoReframe (Smart Reframe) filter tests
    // =========================================================================

    #[test]
    fn test_auto_reframe_without_analysis_data_returns_null() {
        // Given an AutoReframe effect with no analysis data
        let effect = Effect::new(EffectType::AutoReframe);
        // When we build the filter
        let filter = effect.to_filter_body();
        // Then it should return null (analysis required first)
        assert_eq!(filter, "null");
    }

    #[test]
    fn test_auto_reframe_with_invalid_json_returns_null() {
        // Given an AutoReframe effect with invalid JSON analysis data
    ...
    #[test]
    fn test_auto_reframe_with_single_keyframe() {
        // Given analysis data with a single keyframe (static crop)
        let mut effect = Effect::new(EffectType::AutoReframe);
        ...
        assert!(
            filter.contains("crop=608:1080:656:0"),
            "Expected static crop, got: {}",
            filter
        );
    }

    #[test]
    fn test_auto_reframe_with_multiple_keyframes() {
        // Given analysis data with multiple keyframes
        let mut effect = Effect::new(EffectType::AutoReframe);
        let data = serde_json::json!({
            "crop_w": 608,
            "crop_h": 1080,
        ...
        assert!(
            filter.starts_with("crop=608:1080:"),
            "Expected crop with dimensions, got: {}",
            filter
        );
        // And should contain interpolation logic (if/lt)
        assert!(
            filter.contains("if(lt(t"),
            "Expected piecewise interpolation, got: {}",
            filter
        );
    }

    #[test]
    fn test_auto_reframe_zoom_reduces_crop_dimensions() {
        // Given a 20% zoom applied to the reframe
        let mut effect = Effect::new(EffectType::AutoReframe);
        let data = serde_json::json!({
            "crop_w": 1000,
            "crop_h": 1000,
    ...
    #[test]
    fn test_auto_reframe_zoom_clamp() {
        // Given a zoom value exceeding maximum (50)
        let mut effect = Effect::new(EffectType::AutoReframe);
        let data = serde_json::json!({
            "crop_w": 1000,
            "crop_h": 1000,
    ...
    #[test]
    fn test_auto_reframe_no_keyframes_uses_center_crop() {
        // Given analysis data with empty keyframes array
        let mut effect = Effect::new(EffectType::AutoReframe);
        let data = serde_json::json!({
            "crop_w": 608,
            "crop_h": 1080,
        ...
        assert!(
            filter.contains("(iw-608)/2") && filter.contains("(ih-1080)/2"),
            "Expected center crop fallback, got: {}",
            filter
        );
    }

    #[test]
    fn test_auto_reframe_effect_category() {
        // Given an AutoReframe effect
        let effect = Effect::new(EffectType::AutoReframe);
        // Then its category should be AI
        assert_eq!(effect.effect_type.category(), EffectCategory::Ai);
    }

    #[test]
    fn test_auto_reframe_default_params() {
        // Given a new AutoReframe effect
        let effect = Effect::new(EffectType::AutoReframe);
        // Then default params should be set correctly
        assert_eq!(
            effect.get_string("target_aspect"),
            Some("9:16".to_string()),
            "Default target_aspect should be '9:16'"
        );
        assert_eq!(
            effect.get_float("smoothing"),
            Some(30.0),
            "Default smoothing should be 30.0"
        );
        assert_eq!(
            effect.get_float("zoom"),
            Some(0.0),
        ...
        assert_eq!(
            effect.get_string("detection_mode"),
            Some("center".to_string()),
            "Default detection_mode should be 'center'"
        );
        assert_eq!(
            effect.get_string("analysis_data"),
            Some(String::new()),
            "Default analysis_data should be empty"
        );
    }

    #[test]
    fn test_auto_reframe_is_ffmpeg_compatible() {
        // Given an AutoReframe effect
        let effect = Effect::new(EffectType::AutoReframe);
        // Then it should be FFmpeg compatible
        assert!(
            effect.is_ffmpeg_compatible(),
            "AutoReframe should be FFmpeg compatible"
        );
    }

    #[test]
    fn test_auto_reframe_zero_crop_dimensions_returns_null() {
        // Given analysis data with zero crop dimensions
        let mut effect = Effect::new(EffectType::AutoReframe);
        let data = serde_json::json!({
            "crop_w": 0,
            "crop_h": 1080,
